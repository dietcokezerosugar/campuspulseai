  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400">Overview of campus health and issue resolution</p>
      </div>

      {/* KPI Cards */}
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <StatCard title="Total Reports" value={totalIssues} icon={TrendingUp} color="bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300" />
        <StatCard title="Suggestions" value={suggestions} icon={Lightbulb} color="bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400" />
        <StatCard title="Open Issues" value={openIssues} icon={AlertCircle} color="bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400" />
        <StatCard title="Resolved" value={resolvedIssues} icon={CheckCircle} color="bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400" />
        <StatCard title="Critical Pending" value={criticalIssues} icon={Clock} color="bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400" />
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        {/* Status Pie Chart */}
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
          <h3 className="font-bold text-slate-800 dark:text-white mb-6">Resolution Status</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
          ...
          <div className="flex justify-center space-x-4 mt-2">
            {statusData.map(d => (
              <div key={d.name} className="flex items-center text-xs text-slate-600 dark:text-slate-400">
...
const StatCard = ({ title, value, icon: Icon, color }: any) => (
  <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between transition-colors">
    <div>
      <p className="text-slate-500 dark:text-slate-400 text-xs uppercase font-bold tracking-wider mb-1">{title}</p>
